...
void	CCDroneMng::EDROOM_CTX_Top_0::FReplayDroneReady()
	...
	do
	{
		...
		switch(edroomCurrentTrans.localId)
		{
			...
			case (ExecTC):
				...
				if( GExecFlightPlan() )
				{
					...
					//Next State is FlyPlan
					edroomNextState = FlyPlan;
				 } 
			...
			case (CtrlAlorithm):
				...
				if( GFlightPlanDone() )
				{
					...
					//Branch taken is CtrlAlorithm_PlanDone
					edroomCurrentTrans.localId =
				...
				else
				{
					...
					//Branch taken is CtrlAlorithm_ProgNextCtrl
					edroomCurrentTrans.localId =
		...
		switch(edroomNextState)
		{
			...
			case (Ready):
				...
				//Go to the state FlyPlan
			case (FlyPlan):
				//Arrival to state FlyPlan
				edroomCurrentTrans=EDROOMFlyPlanArrival();
				break;
...
TEDROOMTransId CCDroneMng::EDROOM_SUB_Top_0::EDROOMReadyArrival()
{
	...
	// Leaf SubState  FlyPlan

	// ***********************************************************************



TEDROOMTransId CCDroneMng::EDROOM_SUB_Top_0::EDROOMFlyPlanArrival()
{
		...
		switch(Msg->signal)
		{
			...
			case (EDROOMSignalTimeout): 
				 ...
				 if (*Msg->GetPInterface() == DroneTimer)
				{
					...
					//Next transition is  CtrlAlorithm
					edroomCurrentTrans.localId = CtrlAlorithm;
					edroomCurrentTrans.distanceToContext = 0 ;
					edroomValidMsg=true;
				 }
